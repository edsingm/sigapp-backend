...
- Alterações de catálogo/matriz são transacionais e invalidam imediatamente os caches dos planos afetados. Valores administrativos são estritos: feature é boolean, limites são inteiros `>= 0` ou `-1`, e `ai_budget` aceita número não negativo. Use `plans:audit-entitlements` para auditoria read-only de catálogo, matrizes, aliases, dependências e contabilização de storage.
