...
- Ciclo de vida do tenant: signup público (`SignupController` → `TenantSignupService` → `CreateFullTenantJob`), limpeza de pendentes (`tenants:cleanup-pending`), ativação/suspensão via admin central (`TenantStatus`). O provisionamento cria a chave antes do primeiro `tenant->run()`, executa migrate/seed com exit code obrigatório, valida tabelas/admin/RBAC, diretório central de login e faz um round-trip cifrado antes da única transição final para `active`. O `AdminUserSeeder` sincroniza explicitamente o admin em `tenant_user_directories`, inclusive com model events desabilitados durante o seed. Retentativas preservam schema e chave existentes.
- `Tenant::cancel()` agenda o wipe D90. O wipe durável roda em `WipeTenantJob`, verifica ausência do schema e do prefixo S3, anonimiza o central e só então limpa `encryption_key`/grava `wiped_at`. Ver [`.agents/docs/privacidade-lgpd.md`](./privacidade-lgpd.md).
- PII tenant usa `EncryptedWithTenantKey`; `billing_tax_id` central continua com cast `encrypted`/`APP_KEY`. Chave ausente ou inválida bloqueia somente a superfície sensível em vez de persistir/retornar fallback inseguro.
- Rotinas que atravessam tenants devem usar `chunkById`/cursor. Nunca introduza `Tenant::all()` ou `get()` ilimitado em comandos, scheduler, jobs globais ou rebuilds.
- Inventário de schema: `php artisan sigapp:schema-status --fail-on-drift` compara migrations centrais e cada schema provisionado, de forma paginada, e publica o fingerprint consumido pela readiness.
