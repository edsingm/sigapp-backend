...
- RAG: `AiDocumentChunk`/`AiDocumentEmbedding` guardam embeddings JSON e `AiEmbeddingService` calcula similaridade de cosseno na aplicação, com limiar configurável (`AI_EMBEDDING_MIN_SIMILARITY`). O upload de documento despacha `IndexDocumentEmbeddingJob`; todos os novos vetores são preparados antes e o índice ativo só é substituído em uma transaction completa, portanto falha de provider/banco deve preservar o índice anterior e nunca gravar vetor nulo/zerado. Provider e modelo de embedding são `AI_EMBEDDING_PROVIDER` e `AI_EMBEDDING_MODEL`.
