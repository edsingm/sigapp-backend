...
- O CI (`.github/workflows/ci.yml`) roda em paralelo: Tests (SQLite, incluindo `composer validate`/`composer audit`), suíte completa com PostgreSQL 16 + pgvector + Redis 7 reais, Pint, PHPStan (`composer analyse`, memory 512M) e build da imagem prod. Em PR a imagem só é construída; em push na `main` ela é publicada em `ghcr.io/edsingm/sigapp-backend:<SHA>`. Staging só começa após todos os jobs verdes. Jobs PHP usam cache de Composer (`composer.lock`). Actions de terceiros ficam fixadas por SHA completo. Não há PRs periódicos de versão do Dependabot; alerts e security updates permanecem habilitados, e atualizações comuns são revisadas em manutenção periódica. Testes exclusivos de infraestrutura ficam em `tests/Feature/Infrastructure/` e devem se marcar como skipped quando o driver não for `pgsql`/`redis`; nunca substitua essa cobertura por mocks.
