...
| **Observabilidade** | Sentry Laravel (`sentry/sentry-laravel`) em staging e produção; ambientes separados por tag e release vinculada ao SHA da imagem |
...
- Sentry roda em staging e produção com `SENTRY_LARAVEL_DSN` obrigatório no Dokploy. `SENTRY_ENVIRONMENT` é fixado pelo Compose e a release usa `APP_REVISION` (SHA da imagem), salvo override explícito por `SENTRY_RELEASE`. Erros usam sample rate 100%; tracing usa 100% em staging e 10% em produção. Por LGPD, PII, bindings SQL e tracing de conteúdo da IA ficam desabilitados nos deploys.
