...
- **Staging (`docker-compose.staging.yml`)**: target `staging` (alias de `prod`); mesma forma do prod (porta 80, healthcheck, envs `${VAR:?}`), rede do Postgres via `DATABASE_DOCKER_NETWORK`. Stripe test, Redis/S3/`APP_KEY` isolados.
...
- **Runbook de deploy:** `docs/deploy-dokploy.md` (Dokploy + Compose + `sigapp-release` via SSH). `docs/deploy-coolify.md` está **obsoleto**. **Cenário B:** auto-deploy de `main` só em staging (`api.staging.sigapp.com.br`); produção (`api.sigapp.com.br`, Stripe live) é deploy **manual** da mesma revisão. Após cada deploy, nos dois ambientes, rodar `/usr/local/bin/sigapp-release` no container `back` **novo** (prod: `sigapp-backend-j8lepv-back-1`; staging: descobrir com `docker ps`). Merge na `main` **não** é go-live.
