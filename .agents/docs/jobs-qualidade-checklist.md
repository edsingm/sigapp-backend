...
- **Deploy:** somente **Dokploy** + Compose na VPS (Cenário B: auto-deploy de `main` só em staging; prod manual da mesma revisão + `sigapp-release` no container novo). Runbook: [`docs/deploy-dokploy.md`](../../docs/deploy-dokploy.md). Não documente nem reintroduza Coolify, AWS ECS/EKS ou outro orquestrador.
