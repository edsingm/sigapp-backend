...
class ViabilidadeUnificadoService
{
    ...
    private function montarParametros(?Viabilidade $v): array
    {
        ...
        return [
            ...
            'custoAreaComum' => (float) ($v->area_comum ?? $defaults['area_comum']),
            'percentualContrapartidas' => ($v->contrapartidas ?? $defaults['contrapartidas']) / 100,
            'canteiroMensal' => (float) ($v->canteiro_mensal ?? $defaults['canteiro_mensal']),
            'moAdministrativa' => (float) ($v->mo_administrativa ?? $defaults['mo_administrativa']),
            ...
            'standVendas' => (float) ($v->stand_vendas ?? $defaults['stand_vendas']),
            'mobiliaDecoracao' => (float) ($v->mobilia_decoracao ?? $defaults['mobilia_decoracao']),
            'construcaoStandMesesAntesLancamento' => (int) ($v->construcao_stand_meses_antes_lancamento ?? ($defaults['construcao_stand_meses_antes_lancamento'] ?? 0)),
            ...
            'bonusEquipeComercial' => (float) ($v->bonus_equipe_comercial ?? ($defaults['bonus_equipe_comercial'] ?? 0.0)),
            ...
            'marketingLancamento' => $v->marketing_lancamento !== null ? ((float) $v->marketing_lancamento / 100) : null,
            'custoItbiIptu' => ($v->itbi_iptu ?? $defaults['itbi_iptu']) / 100,
            'custoRegistro' => (float) ($v->registro ?? $defaults['registro']),
            'custoContratacaoCef' => (float) ($v->custo_contratacao_cef ?? $v->medicao_contratacao ?? $defaults['custo_contratacao_cef']),
            'custoMedicaoCef' => (float) ($v->custo_medicao_cef ?? $defaults['custo_medicao_cef']),
            'custoContratosCef' => (float) ($v->contratos_cef ?? $defaults['contratos_cef']),
            ...
            'taxaJurosPj' => ($v->taxa_juros_pj ?? $defaults['taxa_juros_pj']) / 100,
            'carenciaPjMeses' => (int) ($v->carencia_pj_meses ?? $defaults['carencia_pj_meses']),
            'amortizacaoPjParcelas' => (int) ($v->amortizacao_pj_parcelas ?? $defaults['amortizacao_pj_parcelas']),
            ...
            'incorporacaoRi' => ($v->incorporacao_ri ?? $defaults['incorp_ri']) / 100,
            'incorporacaoEntrega' => ($v->incorporacao_entrega ?? $defaults['incorp_entrega']) / 100,
            'incorporacaoAteLancamento' => ($v->incorporacao_ate_lancamento ?? $defaults['incorp_ate_lancamento']) / 100,
            ...
            'custoMedicaoContratacao' => (float) ($v->medicao_contratacao ?? $v->custo_contratacao_cef ?? $defaults['custo_contratacao_cef'] ?? 0),
